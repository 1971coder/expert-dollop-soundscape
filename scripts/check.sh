...
if [[ -n "${XCODE_PROJECT}" && -d "${XCODE_PROJECT}" ]]; then
  ...
  if [[ -z "${XCODE_DESTINATION}" ]]; then
    echo "[check] no iOS simulator destination available; skipping build" >&2
    echo "[check] install a simulator runtime in Xcode > Settings > Components, or export XCODE_DESTINATION" >&2
    exit 0
  fi
  if command -v xcodebuild >/dev/null 2>&1; then
    echo "[check] destination: ${XCODE_DESTINATION}"
